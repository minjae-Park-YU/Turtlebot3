...
class Critic(nn.Module):
    def __init__(self, state_dim, action_dim):
        ...
        self.fa1 = nn.Linear(512 + action_dim, 512)
...
class Actor(nn.Module):
    def __init__(self, state_dim, action_dim, action_limit_v, action_limit_w):
        ...
        self.fa1 = NoisyLinear(state_dim, 512)
        self.fa1.weight.data = fanin_init(self.fa1.weight.data.size())

        self.fa2 = NoisyLinear(512, 512)
        self.fa2.weight.data = fanin_init(self.fa2.weight.data.size())

        self.fa3 = NoisyLinear(512, action_dim)
   ...
   # for ensemble in range(10):
    ...
    for ep in range(MAX_EPISODES):
        ...
        for step in range(MAX_STEPS):
            ...
            if ram.len >= before_training * MAX_STEPS * 4 and is_training and not ep % 10 == 0:
        ...
        if temp_list[-1][3]:  # step 50 이후 충돌한 경우
            ...
            #setHER(temp_list, round(len(temp_list[:])*0.5), reward_mode)
            #setHER(temp_list, round(len(temp_list[:])*0.4), reward_mode)
        ...
        if ep % 1000 == 0 and ep > 0:
            df_name = "reward_ep" + str(ep) + " Ensemble_" + str(ensemble) + ".csv"
            df1_name = "new_rewad_ep" + str(ep) + " Ensemble " + str(ensemble) + ".csv"
            print("data saving...")
            ...
            df1.to_csv(dirPath + "/Rewards/" + world + '/Original Rewards/' + df_name)
            df2.to_csv(dirPath + "/Rewards/" + world + '/New Rewards/' + df1_name)
