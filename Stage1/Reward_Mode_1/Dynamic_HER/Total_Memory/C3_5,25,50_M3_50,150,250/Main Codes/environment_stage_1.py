...
class Env():
    ...
    def getState(self, scan, past_action):
        ...
        # (22.08.11. park) 조건문을 이용하여 장애물과의 최소 거리 및 방향이 추가된 상태 반환
        if stage_mode == 1:
            return scan_range + [heading, current_distance], done, self.get_goalbox, cord
        if stage_mode == 2:
            return scan_range + [heading, current_distance, obstacle_min_range, obstacle_angle], done, self.get_goalbox, cord


    def setReward(self, state, done):
        # (22.08.15. park) current distance 부분 잘못된거 수정
        current_distance = state[13]
        # heading = state[-2]
