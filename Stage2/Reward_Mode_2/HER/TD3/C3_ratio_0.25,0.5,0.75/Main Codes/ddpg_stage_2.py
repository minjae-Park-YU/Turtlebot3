...
class MLPNetwork(nn.Module):
    def __init__(self, state_dim, action_dim, hidden_size=250):
        ...
        self.fa1 = nn.Linear(action_dim, hidden_size)
        self.fa1.weight.data = fanin_init(self.fa1.weight.data.size())
        
        self.fca1 = nn.Linear(hidden_size * 2, hidden_size * 2)
        self.fca1.weight.data = fanin_init(self.fca1.weight.data.size())
        
        self.fca2 = nn.Linear(hidden_size * 2, 1)
        self.fca2.weight.data.uniform_(-0.003, 0.003)
...
class DoubleQCritic(nn.Module):
    ...
    def forward(self, state, action):
        return self.network1(state, action), self.network2(state, action)
        
    def Q1(self, state, action):
        return self.network1(state, action)
...
class Trainer:
    ...
    def __init__(self, state_dim, action_dim, action_limit_v, action_limit_w, ram, ACTION_DIMENSION, policy_freq=2):
        self.loss_actor = 0
        self.state_dim = state_dim
        self.action_dim = action_dim
        self.action_limit_v = action_limit_v
        ...
        # (22.09.28. park) TD3 noise
        self.td3_noise_clip = 0.5
        self.td3_policy_noise = 0.2
        ...
        self.critic = DoubleQCritic(self.state_dim, self.action_dim, hidden_size=250)
        self.target_critic = DoubleQCritic(self.state_dim, self.action_dim, hidden_size=250)
    ...
    def optimizer(self, step):
        ...
        v_noise = (torch.randn_like(a_sample[0]) * self.td3_policy_noise * ACTION_V_MAX).clamp(-self.td3_noise_clip * ACTION_V_MAX, self.td3_noise_clip * ACTION_V_MAX)
        w_noise = (torch.randn_like(a_sample[1]) * self.td3_policy_noise * ACTION_W_MAX).clamp(-self.td3_noise_clip * ACTION_W_MAX, self.td3_noise_clip * ACTION_W_MAX)
        
        a_target = self.target_actor(new_s_sample).detach()
        a_target[0] = (a_target[0] + v_noise).clamp(-ACTION_V_MAX, ACTION_V_MAX)
        a_target[1] = (a_target[1] + w_noise).clamp(-ACTION_W_MAX, ACTION_W_MAX)
        
        # next_value = torch.squeeze(self.target_critic.forward(new_s_sample, a_target).detach())
        next_value1, next_value2 = self.target_critic.forward(new_s_sample, a_target)
        ...
        y_predicted1, y_predicted2 = self.critic.forward(s_sample, a_sample)
        ...
        #print("Q1 Loss: ", F.smooth_l1_loss(y_predicted1, y_expected))
        #print("Q2 Loss: ", F.smooth_l1_loss(y_predicted2, y_expected))
        
        loss_critic = F.smooth_l1_loss(y_predicted1, y_expected) + F.smooth_l1_loss(y_predicted2, y_expected)
        ...
        if step % self.policy_freq == 0:
            ...
            loss_actor = -1 * torch.sum(self.critic.Q1(s_sample, pred_a_sample))
        ...
        if self.loss_actor != 0:
            return self.loss_actor.detach().numpy().tolist(), loss_critic.detach().numpy().tolist()
        else:
            return 0, loss_critic.detach().numpy().tolist()
    ...
    for ep in range(MAX_EPISODES):
        ...
        for step in range(MAX_STEPS):
            ...
            if ram.len >= MAX_STEPS * 4 and is_training:
                ...
                #print("training..........")
                actor_l, critic_l = trainer.optimizer(step)
