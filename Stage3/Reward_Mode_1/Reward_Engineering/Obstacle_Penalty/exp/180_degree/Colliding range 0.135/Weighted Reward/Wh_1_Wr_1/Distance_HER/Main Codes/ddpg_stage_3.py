#!/home/icas/anaconda3/envs/tbtorch/bin/python3
...
def setHER(temp_list, HER_distance, reward_mode):
    ...
    if current_distance > HER_distance:
        ...
        for i in range(terminal_count):
            ...
            if n_goal_box:
                ...
                for k in range(3):
                    ram.add(st, act, new_reward * H_weight, n_st)
                break
            else:
                ram.add(st, act, new_reward * H_weight, n_st)
            del cd, n_cd
    ...
    for ep in range(MAX_EPISODES):
        ...
        for step in range(MAX_STEPS):
            ...
            if goalbox:
                ...
                for _ in range(3):
                    ram.add(state, action, reward_arg[reward_mode - 1] * R_weight, next_state)
        ...
        if temp_list[-1][3]:
            setHER(temp_list, 0.5, reward_mode)
            setHER(temp_list, 1, reward_mode)
            setHER(temp_list, 1.5, reward_mode)
