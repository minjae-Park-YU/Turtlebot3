    ...
    for ep in range(MAX_EPISODES):
        ...
        for step in range(MAX_STEPS):
            ...
            if len(offline_s) > 1000000:
                ...
                df5_name = "offline_datasets ep" + str(ep) + " Ensemble " + str(ensemble) + " data num " + str(offline_count) + ".csv" 
                df5 =  pd.DataFrame(offline_data)
                if not os.path.exists(dirPath + "/Offline Datasets/" + world):
                    os.makedirs(dirPath + "/Offline Datasets/" + world)
                df5.to_csv(dirPath + "/Offline Datasets/" + world + "/" +df5_name) 
